...
//        << "paramSize" << paramSize;
    ...
    if (
        ...
        for (const auto &valueType: valueTypes){
            const int valueSize = typeSizes[valueType];
            if (valueSize > 0 && data.length() < startIndex + valueSize) {
                break;
            }

            QVariant v;
            valueParser(data.mid(startIndex, valueSize), valueType, &v);
            value << v;
            startIndex += valueSize;
        }
...
//        });
    ...
    if (vendorIdInt == invocationResponseVendorId && commandIdInt == invocationResponseCommandId){
        ...
        for (const auto & resultType: resultTypes){
            const int resultSize = typeSizes[resultType];
            if (resultSize > 0 && data.length() < startIndex + resultSize) {
                break;
            }

            QVariant v;
            valueParser(data.mid(startIndex, resultSize), resultType, &v);
            result << v;
