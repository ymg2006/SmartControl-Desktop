...
if __name__ == "__main__":
    ...
    return getProperty("\\x%s\\x%s\\x%s\\x%s"_ba);
            ...
            set_response = property_object.get("Set", {}).get("Response") if property_object.get("Set") else None
            if isinstance(set_response, dict) and set_response.get("gaiaV3"):
                vendor_ID = set_response["gaiaV3"]["vendor_ID"]
                GAIA_command_ID = set_response["gaiaV3"]["GAIA_command_ID"]
                if get_key:
                    get_property_code.append("""    if (vendorId == %s && commandId == %s){
            ...
            result_response = property_object.get("Result", {}).get("Response") if property_object.get("Result") else None
            if (property_object.get("Invocation")
                    and isinstance(result_response, dict)
                    and result_response.get("gaiaV3")):


                vendor_ID = result_response["gaiaV3"]["vendor_ID"]
                GAIA_command_ID = result_response["gaiaV3"]["GAIA_command_ID"]
                if get_key:
                    get_property_code.append("""    if (vendorId == %s && commandId == %s){
            return getPropertyFromVendorCommand(%s, %s);
        }""" % (vendor_ID, GAIA_command_ID, get_key[0], get_key[1]))
